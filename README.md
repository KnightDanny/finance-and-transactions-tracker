...
└── reconciliation.tsx        # Balance gap resolution
...
├── reconciliation/
│   └── engine.ts             # Gap detection & resolution algorithm
├── auth/
│   ├── store.ts              # Zustand auth state (passcode, biometric, lock)
│   ├── storage.ts            # Secure passcode persistence (hashed + salted)
...
- **reconciliation_gaps** — detected balance discrepancies with resolution status
...
> **Note:** This app requires a custom development build — it cannot run in Expo Go due to native SMS module access.
